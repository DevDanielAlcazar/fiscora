    ...
    <div className="print-report-zip hidden">
      ...
      >
        ...
        <p style={{ margin: "2px 0 16px", fontSize: 10, color: "#888" }}>
          Generado el {new Date().toISOString()} &mdash; {r.xmlFilesFound} archivos encontrados,{" "}
          {r.analyzedCount} analizados
        </p>

        <h2>Resumen general</h2>
        <div className="meta-grid">
          <div className="meta-item">
            <span className="meta-label">Archivos encontrados</span>
            <span className="meta-value">{r.xmlFilesFound}</span>
          </div>
          <div className="meta-item">
            <span className="meta-label">Analizados</span>
            <span className="meta-value">{totals.analyzed}</span>
          </div>
          <div className="meta-item">
            <span className="meta-label">Fallidos</span>
            <span className="meta-value">{totals.failed}</span>
          </div>
          <div className="meta-item">
            <span className="meta-label">Sin hallazgos</span>
            <span className="meta-value">{totals.ok}</span>
          </div>
          <div className="meta-item">
            <span className="meta-label">Con críticos</span>
            <span className="meta-value">{totals.withCritical}</span>
          </div>
          <div className="meta-item">
            <span className="meta-label">Con warnings</span>
            <span className="meta-value">{totals.withWarning}</span>
          </div>
          <div className="meta-item">
            <span className="meta-label">Con BOM</span>
            <span className="meta-value">{totals.withBom}</span>
          </div>
          <div className="meta-item">
            <span className="meta-label">Normalización técnica</span>
            <span className="meta-value">{totals.withTechNorm}</span>
          </div>
          <div className="meta-item">
            <span className="meta-label">XML normalizado disponible</span>
            <span className="meta-value">{totals.withNormXml}</span>
          </div>
          <div className="meta-item">
            <span className="meta-label">XMLs con hallazgos bloqueantes</span>
            <span className="meta-value">{priorities.filesWithBlocker}</span>
          </div>
          <div className="meta-item">
            <span className="meta-label">XMLs con prioridad alta</span>
            <span className="meta-value">{priorities.filesWithHigh}</span>
          </div>
        </div>
        ...
        <table>
          <thead>
            <tr>
              <th>Prioridad</th>
              <th>Hallazgos</th>
              <th>Archivos afectados</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>
                <span className="badge-critical">Bloqueante</span>
              </td>
              <td>{priorities.BLOCKER}</td>
              <td>{priorities.filesWithBlocker}</td>
            </tr>
            <tr>
              <td>
              ...
              <td>{priorities.filesWithHigh}</td>
            </tr>
            <tr>
              <td>
                <span className="badge-info">Media</span>
              </td>
            ...
            <tr>
              <td>
                <span className="badge-ok">Informativa</span>
              </td>
              ...
              <tbody>
                {actionGroups.map((g) => (
                  <tr key={g.group}>
                    ...
                    <td>
                      {g.criticalCount > 0 ? (
                        <span className="badge-critical">{g.criticalCount}</span>
                      ) : (
                        "0"
                      )}
                    </td>
                    <td>
                      {g.warningCount > 0 ? (
                        <span className="badge-warning">{g.warningCount}</span>
                      ) : (
                        "0"
                      )}
                    </td>
              ...
              <tbody>
                {topFindings.map((f) => (
                  <tr key={f.code}>
                    ...
                    <td>{f.title}</td>
                    <td>
                      {f.maxSeverity === "CRITICAL" ? (
                        <span className="badge-critical">CRITICAL</span>
                      ) : f.maxSeverity === "WARNING" ? (
            ...
            <table>
              ...
              <tbody>
                {Object.entries(r.summary.byTipoComprobante).map(([tipo, count]) => {
                  const pct =
                    totalAnalyzed > 0 ? ((count / totalAnalyzed) * 100).toFixed(1) : "0.0";
                  return (
                    <tr key={tipo}>
                      <td>{tipo}</td>
                      <td>{count}</td>
            ...
            <table>
              ...
              <tbody>
                <tr>
                  <td>CFDI</td>
                  <td>{docKinds.CFDI}</td>
                  <td>{pct(docKinds.CFDI)}%</td>
                </tr>
                <tr>
                  <td>RETENCIONES</td>
                  <td>{docKinds.RETENCIONES}</td>
                  <td>{pct(docKinds.RETENCIONES)}%</td>
                </tr>
                <tr>
                  <td>Desconocido</td>
                  <td>{docKinds.UNKNOWN}</td>
                  <td>{pct(docKinds.UNKNOWN)}%</td>
                </tr>
              </tbody>
            </table>
          );
          ...
          <>
            ...
            <table>
              ...
              <tbody>
                <tr>
                  <td>Tiempo total</td>
                  <td>{(perf.totalMs / 1000).toFixed(2)}s</td>
                </tr>
                <tr>
                  <td>Tiempo promedio por archivo</td>
                  <td>{perf.avgMs}ms</td>
                </tr>
                <tr>
                  <td>Tiempo máximo</td>
                  <td>
                    {perf.maxMs}ms ({perf.maxMsFile})
                  </td>
                </tr>
                <tr>
                  <td>Tiempo mínimo</td>
                  <td>
                    {perf.minMs}ms ({perf.minMsFile})
                  </td>
                </tr>
                <tr>
                  <td>Total KB procesados</td>
                  <td>{perf.totalKb.toFixed(0)} KB</td>
                </tr>
                <tr>
                  <td>Promedio KB por archivo</td>
                  <td>{perf.avgKb.toFixed(0)} KB</td>
                </tr>
                <tr>
                  <td>Hallazgos originales</td>
                  <td>{perf.totalFindingsOriginal}</td>
                </tr>
                <tr>
                  <td>Hallazgos retornados</td>
                  <td>{perf.totalFindingsReturned}</td>
                </tr>
                <tr>
                  <td>Archivos con truncamiento</td>
                  <td>
                    {perf.filesTruncated > 0 ? (
                      <span className="badge-warning">{perf.filesTruncated}</span>
                    ) : (
                      "0"
                    )}
                  </td>
              ...
              <tbody>
                {affectedFiles.map((af, i) => (
                  <tr key={af.file.name}>
                    ...
                    <td>
                      {af.criticals > 0 ? (
                        <span className="badge-critical">{af.criticals}</span>
                      ) : (
                        "0"
                      )}
                    </td>
                    <td>
                      {af.warnings > 0 ? <span className="badge-warning">{af.warnings}</span> : "0"}
                    </td>
                    <td>
                      <span
                        className={`badge-${af.maxPriority === "BLOCKER" ? "critical" : af.maxPriority === "HIGH" ? "warning" : "info"}`}
                      >
                        {getPriorityLabel(af.maxPriority)}
                      </span>
                    </td>
        ...
        <div className="footer">
          ...
          <p style={{ margin: "4px 0 0" }}>
            No se almacenan los XML fuente en este reporte. Datos sensibles redactados.
          </p>
