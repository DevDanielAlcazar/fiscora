...
export interface PriorityCounts { BLOCKER: number; HIGH: number; MEDIUM: number; LOW: number; filesWithBlocker: number; filesWithHigh: number; }
...
export interface DocKindCounts { CFDI: number; RETENCIONES: number; UNKNOWN: number; }
...
export function getTopFindingCodes(files: ZipFullAnalysisFileResult[], limit = 10): TopFindingRow[] {
...
export function aggregateMassivePerformance(files: ZipFullAnalysisFileResult[]): PerformanceAgg | null {
...
export function getTopAffectedFiles(files: ZipFullAnalysisFileResult[], limit = 10): AffectedFileRow[] {
