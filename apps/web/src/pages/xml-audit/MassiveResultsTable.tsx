        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5 mb-2">
            {[["ALL", "Todos"], ["BLOCKER", "Bloqueantes"], ["HIGH", "Alta prioridad"], ["FAILED", "Fallidos"], ["TRUNCATED", "Truncados"], ["RETENCIONES", "Retenciones"], ["CFDI", "CFDI"]].map(([key, label]) => (
              <button key={key} onClick={() => onMassiveFilterChange(key)} className={`px-2 py-0.5 rounded text-xs font-semibold border transition-all ${massiveFilter === key ? "bg-primary text-primary-foreground border-primary" : "bg-muted text-muted-foreground border-border hover:bg-muted/70"}`}>{label}</button>
            ))}
          </div>
