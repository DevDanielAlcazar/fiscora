  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 flex flex-col">
      <div className="max-w-7xl mx-auto px-4 py-8 w-full flex-1 flex flex-col gap-8">
        ...
        {/* Filters */}
        <form onSubmit={handleSearch} className="bg-gray-900/50 border border-gray-800 rounded-xl p-4 grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-500 uppercase tracking-tighter">Desde</label>
            <input
              type="date"
              value={from}
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-500 uppercase tracking-tighter">Hasta</label>
            <input
              type="date"
              value={to}
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-500 uppercase tracking-tighter">Fuente</label>
            <select
              value={sourceType}
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-500 uppercase tracking-tighter">Estado</label>
            <select
          ...
          <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-12 text-center flex flex-col items-center gap-6">
            ...
            <div>
              ...
              <p className="text-gray-400 max-w-sm mx-auto">
                No se encontraron registros en la ventana seleccionada. Inicia una nueva auditoría para comenzar.
              </p>
            </div>
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
              <div className="bg-gray-800/40 p-4 rounded-xl border border-gray-800">
                <span className="block text-[10px] text-gray-500 font-bold uppercase mb-1">Total Análisis</span>
                <span className="text-2xl font-bold text-gray-100">{data.totals.records}</span>
              </div>
              <div className="bg-gray-800/40 p-4 rounded-xl border border-gray-800">
                <span className="block text-[10px] text-green-500 font-bold uppercase mb-1">OK</span>
                <span className="text-2xl font-bold text-green-400">{data.totals.ok}</span>
              </div>
              <div className="bg-gray-800/40 p-4 rounded-xl border border-gray-800">
                <span className="block text-[10px] text-yellow-500 font-bold uppercase mb-1">Advertencias</span>
                <span className="text-2xl font-bold text-yellow-400">{data.totals.warning}</span>
              </div>
              <div className="bg-gray-800/40 p-4 rounded-xl border border-gray-800">
                <span className="block text-[10px] text-red-500 font-bold uppercase mb-1">Críticos</span>
                <span className="text-2xl font-bold text-red-400">{data.totals.critical + data.totals.failed}</span>
              </div>
              <div className="bg-gray-800/40 p-4 rounded-xl border border-gray-800">
                <span className="block text-[10px] text-blue-500 font-bold uppercase mb-1">Lotes ZIP</span>
                <span className="text-2xl font-bold text-blue-400">{data.totals.batches}</span>
              </div>
              <div className="bg-gray-800/40 p-4 rounded-xl border border-gray-800">
                <span className="block text-[10px] text-purple-500 font-bold uppercase mb-1">Normalizados</span>
                <span className="text-2xl font-bold text-purple-400">{data.totals.recordsWithNormalizedXml}</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6 flex flex-col gap-4">
                <h3 className="font-bold text-sm uppercase text-gray-500">Distribución de Prioridades</h3>
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-gray-800">
                      {data.priorities.sort((a,b) => b.findings - a.findings).map((p) => (
                        <tr key={p.priority} className="hover:bg-gray-800/20">
                          <td className="py-2 flex items-center gap-2">
                            <span className={`w-2 h-2 rounded-full ${
                              p.priority === 'BLOCKER' ? 'bg-red-500' :
                          ...
                          <td className="py-2 text-right font-bold text-xs">{p.recordsAffected}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6 flex flex-col gap-4">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-gray-800">
                      {data.documentKinds.sort((a,b) => b.count - a.count).map((dk) => (
            ...
            <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6 space-y-4">
              <h3 className="font-bold text-sm uppercase text-gray-500">Hallazgos más recurrentes</h3>
              <div className="overflow-x-auto">
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {data.topFindingCodes.map((fc) => (
                      <tr key={fc.code} className="hover:bg-gray-800/20">
                        ...
                        <td className="py-3 max-w-xs truncate" title={fc.title}>{fc.title}</td>
                        <td className="py-3 text-right font-bold">{fc.count}</td>
                        <td className="py-3 text-right text-gray-400">{fc.recordsAffected} recs</td>
                        <td className="py-3 text-center">
                          <span className={`px-2 py-0.5 rounded-[4px] text-[9px] font-bold ${
                            fc.severityMax === 'CRITICAL' ? 'bg-red-900/30 text-red-400 border border-red-900/50' :
                            fc.severityMax === 'WARNING' ? 'bg-yellow-900/30 text-yellow-400 border border-yellow-900/50' :
            ...
            <div className="space-y-4">
               <div className="flex items-center justify-between">
                 ...
                 <button onClick={() => navigate("/modules/xml-audit/history")} className="text-xs text-primary hover:underline font-bold">Ver historial completo</button>
               </div>
                    ...
                    <tbody className="divide-y divide-gray-800">
                      {data.recentRecords.map((r) => (
                        <tr key={r.id} className="hover:bg-gray-800/20">
                          <td className="px-4 py-3 whitespace-nowrap text-gray-300">{formatDate(r.createdAt)}</td>
                          <td className="px-4 py-3">
                            <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${r.sourceType === 'ZIP' ? 'bg-purple-900/40 text-purple-300' : 'bg-blue-900/40 text-blue-300'}`}>
                              {r.sourceType}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <span className={`font-bold ${r.analysisStatus === 'FAILED' ? 'text-red-500' : 'text-green-500'}`}>
                              {r.analysisStatus === 'FAILED' ? 'ERROR' : 'OK'}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-gray-400">{r.documentKind}</td>
                          <td className="px-4 py-3">
                            <div className="font-mono text-[10px] text-gray-500">{r.rfcEmisor || '—'} ➔ {r.rfcReceptor || '—'}</div>
                          </td>
                          <td className="px-4 py-3 text-right">
                             <div className="flex flex-col items-end gap-0.5">
                               <span className="font-bold">{r.findingsCount}</span>
                               <div className="flex gap-0.5">
                                 {r.criticalCount > 0 && <span className="w-1 h-1 rounded-full bg-red-500" />}
                                 {r.warningCount > 0 && <span className="w-1 h-1 rounded-full bg-yellow-500" />}
                               </div>
                             </div>
                          </td>
                          <td className="px-4 py-3 text-center">
                            <button onClick={() => handleViewRecord(r.id)} className="text-primary hover:underline font-bold">Ver</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pb-12">
               <button onClick={() => navigate("/modules/xml-audit")} className="p-4 bg-gray-900 border border-gray-800 rounded-2xl hover:border-primary transition-all group">
               ...
               <button onClick={() => navigate("/modules/xml-audit/history")} className="p-4 bg-gray-900 border border-gray-800 rounded-2xl hover:border-primary transition-all group">
                 <div className="flex flex-col items-center gap-2">
               ...
               <button onClick={() => navigate("/modules/xml-audit/history/batches")} className="p-4 bg-gray-900 border border-gray-800 rounded-2xl hover:border-primary transition-all group">
                 <div className="flex flex-col items-center gap-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="space-y-4">
                      <div className="space-y-1">
                        ...
                        <p className="text-sm font-mono text-gray-300 break-all">{recordDetail.uuid || recordDetail.sourceFilename || '—'}</p>
                      </div>
                      <div className="space-y-1">
