...
import { getXmlAnalyses, getXmlAnalysisDetail, exportXmlAnalyses, type XmlAnalysisListItem, type XmlAnalysisDetailResponse, type XmlAnalysesQuery } from "../api/admin";
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="p-4 rounded-xl border border-border bg-card space-y-3">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <input placeholder="Tipo comprobante" value={fTipo} onChange={e => setFTipo(e.target.value)} className="text-sm px-3 py-1.5 rounded border border-border bg-background text-foreground" />
            <select value={fRiesgo} onChange={e => setFRiesgo(e.target.value)} className="text-sm px-3 py-1.5 rounded border border-border bg-background text-foreground">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleExportCsv} className="px-4 py-1.5 rounded-lg border border-border text-foreground font-semibold text-sm hover:bg-muted transition-all ml-auto">Exportar CSV</button>
          </div>
        </div>
              ...
              <tbody>
                {items.map((r) => (
                  <tr key={r.id} className="border-b border-border/30 hover:bg-muted/20">
                    ...
                    <td className="py-1.5 px-3 max-w-[120px] truncate" title={r.userEmail}>{r.userEmail}</td>
                    ...
                    <td className="py-1.5 px-3 text-center">
                      <button onClick={() => handleDetail(r.id)} className="text-primary font-semibold hover:underline">
            ...
            {detailLoading ? (
              ...
              <>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-1 text-xs">
                  ...
                  <div className="flex justify-between py-1 border-b border-border/30"><span className="text-muted-foreground">Nombre emisor</span><span>{detail.nombreEmisor ?? "—"}</span></div>
                  <div className="flex justify-between py-1 border-b border-border/30"><span className="text-muted-foreground">RFC receptor</span><span className="font-mono">{detail.rfcReceptor ?? "—"}</span></div>
                  <div className="flex justify-between py-1 border-b border-border/30"><span className="text-muted-foreground">Nombre receptor</span><span>{detail.nombreReceptor ?? "—"}</span></div>
                  <div className="flex justify-between py-1 border-b border-border/30"><span className="text-muted-foreground">Fecha</span><span>{detail.fecha ?? "—"}</span></div>
                  <div className="flex justify-between py-1 border-b border-border/30"><span className="text-muted-foreground">Total</span><span>{detail.total ?? "—"}</span></div>
                  <div className="flex justify-between py-1 border-b border-border/30"><span className="text-muted-foreground">Subtotal</span><span>{detail.subtotal ?? "—"}</span></div>
                  <div className="flex justify-between py-1 border-b border-border/30"><span className="text-muted-foreground">Moneda</span><span>{detail.moneda ?? "—"}</span></div>
                  <div className="flex justify-between py-1 border-b border-border/30"><span className="text-muted-foreground">Versión</span><span>{detail.version ?? "—"}</span></div>
                  ...
                  {detail.zipEntryIndex != null && <div className="flex justify-between py-1 border-b border-border/30"><span className="text-muted-foreground">ZIP entry index</span><span className="font-mono">{detail.zipEntryIndex}</span></div>}
                  <div className="flex justify-between py-1 border-b border-border/30 col-span-2"><span className="text-muted-foreground">Creado</span><span>{fmt(detail.createdAt)}</span></div>
