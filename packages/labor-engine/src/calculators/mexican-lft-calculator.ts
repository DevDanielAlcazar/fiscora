import { LaborCalculationInput, LaborCalculationResult, LaborConceptBreakdown } from "../types.js";
 ...
 */
export function calculateLabor(input: LaborCalculationInput): LaborCalculationResult {
  const ruleSet = input.ruleSet || getDefaultRuleSet();
  const concepts: LaborConceptBreakdown[] = [];
  ...
  const totalPerceptions = concepts.reduce(
    (acc, c) => acc + (c.type === "PERCEPTION" ? c.amount : 0),
    0,
  );
